...
public class GamePanel extends JPanel implements MouseListener {
    ...
    public void init() {
        ...
        for (int i = 1; i <= acakan.length; ++i) {
            acakan[i - 1] = new Tumpukan(ACAKAN_POSITION.x + MARGIN * (i - 1), ACAKAN_POSITION.y) {
                @Override
                    ...
                    if (!this.isEmpty()) {
                        for (int k = 0; k < this.getTumpukan().size(); k++) {
                            if (k + 1 == this.getTumpukan().size()) {
                                g.drawImage(this.topCard().getImage(), 0, yOffset, IMG_WIDTH, IMG_HEIGHT, null);
                                yOffset += 20;
                            } else {
                                g.drawImage(this.topCard().getBackImage(), 0, yOffset, IMG_WIDTH, IMG_HEIGHT, null);
                                yOffset += 20;
                            }
                        }
                    ...
                    for (Tumpukan x : acakan) {
                        if (x == target) {
                            if (x.isEmpty()) {
                                ...
                                if (x.topCard().getNilai() - 1 == ((Tumpukan) source).topCard().getNilai()) {
                                    x.push(((Tumpukan) source).pop());
                                }
                            }
                            if (!((Tumpukan) source).isEmpty()) {
